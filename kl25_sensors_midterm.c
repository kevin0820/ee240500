...
float acc_to_speed (int acc,float speed_init) {
	float speed=acc*0.01;
	return speed;
	/*float sum = 0 ;
	for (i = 0; i < 0.01; i += (b - a) / 1000) {
...
float speed_to_disp (float disp_init,float speed) {
	float disp=speed*0.01+disp_init;
	return disp;
}
...
int main (int argc, char **argv) {
	...
	float displace_x =0, displace_y =0;
	...
	while (1) {
		...
		if (xdata_fp = fopen(filename, "w")) {
			...
			for (i=0; i<sample; i++) {
				...
				if(disp_control=10) {
					speed_x = acc_to_speed (acc_x/100*100,speed_x);
					displace_x = speed_to_disp (displace_x,speed_x);
					speed_y = acc_to_speed (acc_y/200*100,speed_y);
					displace_y = speed_to_disp (displace_y,speed_y);
				}
                                else {
					displace_x = 0;
					speed_x = 0;
				...
				#ifdef DEBUG
				printf("X command receive: (%d %d %d %f %f %.2f %.2f %.2f %.2f) t=%d\n", data_x, data_y, data_z, theta, data_TSI, displace_x, speed_x , displace_y, speed_y ,timecycle);
				#endif

				if (i != sample-1) {
					fprintf(xdata_fp, "[%d, %d, %d, %.2f, %.2f],\n", acc_x, acc_y, acc_z, displace_x, displace_y);
